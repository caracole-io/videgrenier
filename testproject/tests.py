...
class VideGrenierTests(TestCase):
    ...
    def test_views_status(self):
        ...
        self.assertEqual(
            self.client.get(reverse("videgrenier:reservation-detail")).status_code,
            200,
        )
        self.assertEqual(
            self.client.get(reverse("videgrenier:reservation")).status_code,
            200,
        )
        self.assertEqual(
            self.client.get(reverse("videgrenier:reservation-delete")).status_code,
            200,
        )
    ...
    def test_reservation_update_view(self):
        ...
        self.client.get(
            reverse(
                "videgrenier:reservation-moderate",
                kwargs={"pk": reservation.pk, "accepte": 0},
            ),
        )
        ...
        # d accept la réservation: la réservation est acceptée,
        # a & c reçoivent des mails correspondants
        self.client.get(
            reverse(
                "videgrenier:reservation-moderate",
                kwargs={"pk": reservation.pk, "accepte": 1},
            ),
        )
    ...
    def test_dates(self):
        ...
        self.assertLess(
            settings.DATES_VIDE_GRENIER["close"],
            settings.DATES_VIDE_GRENIER["event"],
        )
